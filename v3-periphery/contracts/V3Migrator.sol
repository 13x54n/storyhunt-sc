...
import './interfaces/IStoryHuntV2Pair.sol';
...
contract V3Migrator is IV3Migrator, PeripheryImmutableState, PoolInitializer, Multicall, SelfPermit {
    ...
    function migrate(MigrateParams calldata params) external override {
        ...
        IStoryHuntV2Pair(params.pair).transferFrom(msg.sender, params.pair, params.liquidityToMigrate);
        (uint256 amount0V2, uint256 amount1V2) = IStoryHuntV2Pair(params.pair).burn(address(this));
        ...
        if (amount0V3 < amount0V2) {
            ...
            if (params.refundAsETH && params.token0 == WIP9) {
                IWIP9(WIP9).withdraw(refund0);
        ...
        if (amount1V3 < amount1V2) {
            ...
            if (params.refundAsETH && params.token1 == WIP9) {
                IWIP9(WIP9).withdraw(refund1);
