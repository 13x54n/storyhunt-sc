import { abi as IStoryHuntPoolABI } from '@uniswap/v3-core/artifacts/contracts/interfaces/IStoryHuntPool.sol/IStoryHuntPool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IStoryHuntPool, IWIP9, MockTimeSwapRouter, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  async function exactOutput(tokens: string[]): Promise<ContractTransaction> {
    ...
    if (inputIsWIP9) data.push(router.interface.encodeFunctionData('refundETH'))
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestStoryHuntCallee')
