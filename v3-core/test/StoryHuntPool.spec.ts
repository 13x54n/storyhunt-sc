...
import { TestStoryHuntSwapPay } from '../typechain/TestStoryHuntSwapPay'
...
import { TestStoryHuntReentrantCallee } from '../typechain/TestStoryHuntReentrantCallee'
...
describe('StoryHuntPool', () => {
  ...
  let factory: StoryHuntFactory
  let pool: MockTimeStoryHuntPool

  let swapTarget: TestStoryHuntCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeStoryHuntPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestStoryHuntReentrantCallee')
      ).deploy()) as TestStoryHuntReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestStoryHuntSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestStoryHuntSwapPay')
      underpay = (await underpayFactory.deploy()) as TestStoryHuntSwapPay
