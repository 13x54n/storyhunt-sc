...
import { StoryHuntPoolSwapTest } from '../typechain/StoryHuntPoolSwapTest'
...
describe('StoryHuntPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('StoryHuntPoolSwapTest')
            const tester = (await testerFactory.deploy()) as StoryHuntPoolSwapTest
          ...
          let tester: StoryHuntPoolSwapTest
