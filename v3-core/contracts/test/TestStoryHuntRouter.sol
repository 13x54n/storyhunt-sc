...
contract TestStoryHuntRouter is IStoryHuntSwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IStoryHuntPool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function uniswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IStoryHuntPool(msg.sender).token0() : IStoryHuntPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IStoryHuntPool(pools[0]).token1();
            IStoryHuntPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IStoryHuntPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IStoryHuntPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
